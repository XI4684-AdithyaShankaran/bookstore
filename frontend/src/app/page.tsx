...
export default function Home() {
  ...
  const loadInitialData = useCallback(async () => {
    try {
      ...
      const featuredResponse = await bookService.getFeaturedBooks(6);
      setFeaturedBooks(featuredResponse.books);

      // Load initial books for infinite scroll
      const allBooksResponse = await bookService.searchBooks({ limit: 20 });
      setAllBooks(allBooksResponse.books);
      setHasMore(allBooksResponse.hasMore);
    } catch (error) {
