...
class BookService:
    ...
    def get_books(self, skip: int = 0, limit: int = 25, 
        ...
        try:
            ...
            logger.error(f"❌ Error in books query: {e}")
            return {"books": [], "total": 0, "page": 1, "limit": limit, "hasMore": False}
    
    def get_book_by_id(self, book_id: int) -> Optional[Book]:
        """Single book query with caching"""
        try:
            # Check cache first
    ...
    def search_books(self, search: str, limit: int = 20) -> List[Book]:
        """Search with ranking"""
        try:
            # Check cache first
